...
const setup = ({ rootId, scroll, interval, infinite }: Props) => {
  ...
  const updateSliderBackground = (index: number) => {
  ...
  const onClickPrev = () => {
    ...
    updateSliderBackground(Math.max(indices[0] - itemsPerPage, 0));
  };
  ...
  const onClickNext = () => {
    ...
    updateSliderBackground(
      Math.min(indices[indices.length - 1] + itemsPerPage, items.length - 1),
    );
  };
