    ...
    <footer class="bg-[#ebf0ef]  bottom-0 relative pt-[100px] pb-[50px] border-b-4 border-solid border-[#83ff97]">
      <div class="w-full max-w-[1200px] mx-auto xl:px-0 px-[12px]">
        <div class="grid lg:grid-cols-3 lg:gap-[24px] md:gap-y-0 gap-x-[24px] gap-y-[60px] lg:mb-[150px] mb-[120px] md:grid-cols-2 grid-cols-1">
          <div>
            <p class="text-[16px] Maax-Regular-Font font-normal">
              {props.contents[0]?.first_text}
            </p>
            <a
              href={props.contents[0]?.email_link}
              class="text-[#005046] uppercase text-[14px] tracking-[1px] Maax-Bold-Font font-bold"
            >
          ...
          <div>
            <ul class="lg:flex items-center justify-end flex-wrap flex items-center justify-start lg:justify-end">
              {socialMediaLinks.map((media: SocialMedia, index: number) => (
                <li key={index}>
                  <a
                    ...
                    class="arrow-icon text-[#005046] uppercase text-[14px] tracking-[1px] mr-[30px] Maax-Bold-Font font-bold"
                  >
