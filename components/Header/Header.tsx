...
const navlinkStyles = {
  menuOpen: 'pt-0 pb-0 border-b-0 text-[36px] Noe-Display-Font tracking-[-.5px] leading-[3rem]',
  menuClosed: 'pt-[4px] pb-[2px] border-b-2 text-[14px] Maax-Bold-Font tracking-[-.5px] leading-[3rem]',
};
  ...
  return (
    <header class={`l-header w-full flex items-center z-10 relative fixed top-0 ${headerClass}`}>
      <div class={`mx-auto w-full xl:pr-[0px] pr-[12px] xl:(l-container)`}>
        ...
        <Logo link="https://deco-sites-kaszek.deno.dev/" src="/imgs/logo.svg" className={`z-10 xl:pl-[0px] pl-[12px] ${showMenu ? "color-green-lemon pl-[12px]" : ""}`} width="auto" height={17.95}/>
          <div class="flex-center-end w-full absolute md:(relative)">                    
          <ButtonHamburger showMenu={showMenu} toggleMenu={toggleMenu}/>        
          <ul class={`flex flex-col md:(flex-row) ${
            showMenu ? "bg-[#005046] w-full fixed h-screen bottom-0 flex-center-center" : "hidden uppercase gap-[30px]"
          } md:flex`}>
           {navLinks.map(({ link, title }) => (
              <NavLink link={link} title={title} className={menuItemStyles}/>
            ))}      
          </ul>
          </div>
        </nav>
